...
const AdsCard = () => {
  ...
  const { ads, loading, error } = useSelector((state) => state.news);
  ...
  const filteredadss = ads.slice(1); 
  const filteredads = filteredadss.sort((a,b)=>(new Date(b.created_at) - new Date(a.created_at)))
  const totalPages = Math.ceil(filteredads.length / itemsPerPage);
  const currentNews = filteredads.slice(
