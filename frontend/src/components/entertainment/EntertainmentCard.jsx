...
const EntertainmentCard = () => {
  ...
  const { entertainment, loading, error } = useSelector((state) => state.news);
  ...
  const filteredentertainments = entertainment.slice(1); 
  const filteredentertainment = filteredentertainments.sort((a,b)=>(new Date(b.created_at) - new Date(a.created_at)))
  const totalPages = Math.ceil(filteredentertainment.length / itemsPerPage);
  const currentNews = filteredentertainment.slice(
    ...
    <div className='mb-4'>
      <div className="cards mt-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {currentNews?.slice(1)?.map((item) => (
          <div className='' key={item.entertainment_id}>
