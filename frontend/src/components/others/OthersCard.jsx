...
const OthersCard = () => {
  ...
  const filteredothers = filteredotherss.sort((a,b)=>(new Date(b.created_at) - new Date(a.created_at)))
  const totalPages = Math.ceil(filteredothers.length / itemsPerPage);
  const currentNews = filteredothers.slice(
    ...
    <div className='mb-4'>
      <div className="cards mt-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {currentNews.slice(1)?.map((item) => (
          <div className='' key={item.others_id}>
