...
const NationalCard = () => {
  ...
  const { national, loading, error } = useSelector((state) => state.news);
  ...
  const filteredNationals = national.slice(1); 
  const filteredNational = filteredNationals.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  const totalPages = Math.ceil(filteredNational.length / itemsPerPage);
  const currentNews = filteredNational.slice(
