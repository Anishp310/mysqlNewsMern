...
const EconomicCard = () => {
  ...
  const { economic, loading, error } = useSelector((state) => state.news);
  ...
  const filteredeconomics = economic.slice(1); 
  const filteredeconomic = filteredeconomics.sort((a,b)=>(new Date(b.created_at) - new Date(a.created_at)))
  const totalPages = Math.ceil(filteredeconomic.length / itemsPerPage);
  const currentNews = filteredeconomic.slice(
    ...
    <div className='mb-4'>
      <div className="cards mt-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {currentNews.slice(1)?.map((item) => (
          <div className='' key={item.economic_id}>
