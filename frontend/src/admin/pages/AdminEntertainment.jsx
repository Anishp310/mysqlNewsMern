...
const AdminEntertainment = () => {
  ...
  const getApiData = async () => {
    try {
      const response = await fetch(SummaryApi.getAllEntertainment.url);
...
const handleCreateSubmit = async (data) => {
  ...
  try {
    const response = await fetch(SummaryApi.addEntertainment.url, {
      method: "POST",
      body: formData,
      headers: {
...
const handleUpdateSubmit = async (data) => {
  ...
  try {
    const response = await fetch(`${SummaryApi.updateEntertainment.url}/${editData.entertainment_id}`, {
      method: "PUT",
    ...
    if (response.ok) {
      ...
      setNationalData((prevData) =>
        prevData.map((item) =>
          item.entertainment_id === updatedEntry.entertainment_id ? updatedEntry : item
        )
      );
  ...
  const handleUpdate = (id) => {
    const rowToUpdate = nationalData.find((item) => item.entertainment_id === id);
  ...
  const handleDelete = async (id) => {
    validateToken();
    try {
      const response = await fetch(`${SummaryApi.deleteEntertainment.url}/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
      ...
      if (response.ok) {
        setNationalData((prevData) => prevData.filter((item) => item.entertainment_id !== id));
      } else {
  ...
  const columns = [
    ...
    { name: 'Created At', selector: (row) => new Date(row.created_at).toLocaleString(), sortable: true },
    {
      name: 'Update',
      cell: (row) => (
        <button onClick={() => handleUpdate(row.entertainment_id)} className="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-700 transition-all">
          Update
        </button>
      ),
      sortable: false,
    },
    {
      name: 'Delete',
      cell: (row) => (
        <button onClick={() => handleDelete(row.entertainment_id)} className="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-700 transition-all">
          Delete
        </button>
      ),
      sortable: false,
