...
const AdminAds = () => {
  ...
  const getApiData = async () => {
    try {
      const response = await fetch(SummaryApi.getAllAds.url);
...
const handleCreateSubmit = async (data) => {
  ...
  try {
    const response = await fetch(SummaryApi.addAds.url, {
      method: "POST",
      body: formData,
      headers: {
...
const handleUpdateSubmit = async (data) => {
  ...
  try {
    const response = await fetch(`${SummaryApi.updateAds.url}/${editData.ads_id}`, {
      method: "PUT",
    ...
    if (response.ok) {
      ...
      setNationalData((prevData) =>
        prevData.map((item) =>
          item.ads_id === updatedEntry.ads_id ? updatedEntry : item
        )
      );
  ...
  const handleUpdate = (id) => {
    const rowToUpdate = nationalData.find((item) => item.ads_id === id);
  ...
  const handleDelete = async (id) => {
    validateToken();
    try {
      const response = await fetch(`${SummaryApi.deleteAds.url}/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
      ...
      if (response.ok) {
        setNationalData((prevData) => prevData.filter((item) => item.ads_id !== id));
      } else {
  ...
  const columns = [
    ...
    { name: 'Created At', selector: (row) => new Date(row.created_at).toLocaleString(), sortable: true },
    {
      name: 'Update',
      cell: (row) => (
        <button onClick={() => handleUpdate(row.ads_id)} className="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-700 transition-all">
          Update
        </button>
      ),
      sortable: false,
    },
    {
      name: 'Delete',
      cell: (row) => (
        <button onClick={() => handleDelete(row.ads_id)} className="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-700 transition-all">
          Delete
        </button>
      ),
      sortable: false,
