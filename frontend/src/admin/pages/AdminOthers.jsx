...
const AdminOthers = () => {
  ...
  const getApiData = async () => {
    try {
      const response = await fetch(SummaryApi.getAllOthers.url);
...
const handleCreateSubmit = async (data) => {
  ...
  try {
    const response = await fetch(SummaryApi.addOthers.url, {
      method: "POST",
      body: formData,
      headers: {
...
const handleUpdateSubmit = async (data) => {
  ...
  try {
    const response = await fetch(`${SummaryApi.updateOthers.url}/${editData.others_id}`, {
      method: "PUT",
    ...
    if (response.ok) {
      ...
      setNationalData((prevData) =>
        prevData.map((item) =>
          item.others_id === updatedEntry.others_id ? updatedEntry : item
        )
      );
  ...
  const handleUpdate = (id) => {
    const rowToUpdate = nationalData.find((item) => item.others_id === id);
  ...
  const handleDelete = async (id) => {
    validateToken();
    try {
      const response = await fetch(`${SummaryApi.deleteOthers.url}/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
      ...
      if (response.ok) {
        setNationalData((prevData) => prevData.filter((item) => item.others_id !== id));
      } else {
  ...
  const columns = [
    ...
    { name: 'Created At', selector: (row) => new Date(row.created_at).toLocaleString(), sortable: true },
    {
      name: 'Update',
      cell: (row) => (
        <button onClick={() => handleUpdate(row.others_id)} className="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-700 transition-all">
          Update
        </button>
      ),
      sortable: false,
    },
    {
      name: 'Delete',
      cell: (row) => (
        <button onClick={() => handleDelete(row.others_id)} className="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-700 transition-all">
          Delete
        </button>
      ),
      sortable: false,
